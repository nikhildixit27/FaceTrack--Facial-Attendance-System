...
class MainHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self):
        try:
            ...
            raise RuntimeError(
                f"An SQL error occured from one of the connections. Log: {str(sql_error)}.)")
        ...
        try:
            ...
            if "[DATA]" in data:
                ...
                for row in cursor:
                    isRecordExist = True
                if isRecordExist:
                    cmd = "UPDATE students SET name = '" + \
                        str(register_data[1]) + \
                        "' WHERE id = '" + str(self.id) + "';"
                    cmd2 = "UPDATE students SET section = '" + \
                        str(register_data[2]) + \
                        "' WHERE id = '" + str(self.id) + "';"
                    cmd3 = "UPDATE students SET branch = '" + \
                        str(register_data[3]) + \
                        "' WHERE id = '" + str(self.id) + "';"
                    cmd4 = "UPDATE students SET program = '" + \
                        str(register_data[4]) + \
                        "' WHERE id = '" + str(self.id) + "';"
                    self.conn.execute(cmd)
                else:
                    params = (
                        self.id, register_data[1], register_data[2], register_data[3], register_data[4], 0)
                ...
                if self.wfps == None:
                    ...
                    cmd = "UPDATE students SET previous_marked = " + \
                        today + " WHERE id = '" + str(self.wfps) + "';"
                    cursor = self.conn.execute(cmd)
                    self.conn.commit()
                    self.conn.close()
                    self.wfps = None
                    data_rows = ("Reg. No.", "Name", "Section",
                                 "Branch", "Program", "Previously_Accessed_At")
                    self.udata = (self.udata[0], self.udata[1], self.udata[2], self.udata[3],
                                  self.udata[4], datetime.fromtimestamp(int(today)).strftime("%d-%m-%Y"))
                    header_write = False
                    ...
                    if self.sampleNum <= 100:
                        ...
                        if len(faces) > 1:
                            ...
                            for (x, y, w, h) in faces:
                                self.sampleNum += 1
                                cv2.imwrite("dataSet/student." + str(self.id) + "." +
                                            str(self.sampleNum) + ".jpg", gray[y:y+h, x:x+w])
                    else:
                        ...
                        for imagepath in imagepaths:
                            if ".DS_Store" in imagepath:
                                ...
                                cid = int(os.path.split(imagepath)
                                          [-1].split(".")[1])
                    ...
                    if not self.found:
                        ...
                        if student_data != []:
                            ...
                            if (datetime.now() - datetime.fromtimestamp(student_data[5])).days == 0:
                                self.write_message(
                                    f"{student_data[0]},{student_data[1]},{student_data[2]},{student_data[3]},{student_data[4]},{student_data[5]}")
                            else:
                                self.wfps = cid
                                self.udata = student_data
                                self.write_message(
                                    f"{student_data[0]},{student_data[1]},{student_data[2]},{student_data[3]},{student_data[4]}")
                        else:
            ...
            try:
                ...
                pass
            raise RuntimeError(
                f"An SQL error occured from one of the connections during messaging. Log: {str(sql_error)}.")
